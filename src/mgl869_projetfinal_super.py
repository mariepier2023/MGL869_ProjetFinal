...
import time
from module_super.extractor_commit_super import get_commits
...
def write_commits_to_csv(commit_list, output_file):
    ...
    with open(output_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(['key', 'sha', 'message', 'filename', 'priority'])
        ...
        for commit in commit_list:
            ...
            for bug_file in commit['files']:
                writer.writerow([key, sha, message, bug_file, priority])

def main(version, metadata):
    VERSION = version
    OUTPUT_FILE_NAME = "Bugs_" + VERSION + "_Super.csv"
    ...
    if not os.path.exists(OUTPUT_FILE):
        ...
        for issue in extract_issues(JIRA_SEARCH_FILTER, ["versions", "priority"]):
            affected_versions = [e["name"] for e in issue["fields"]["versions"]]
