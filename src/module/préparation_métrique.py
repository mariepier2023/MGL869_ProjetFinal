...
def prepare_all_metrics(version, inputFilePFName, inputFileUndName, outputFile):
    start_time = time.time()
    # Chemins des fichiers
    file_path = Path(os.path.realpath(__file__)).parent.parent.parent / 'data'
    commit_metriques_PF_file = file_path / inputFilePFName
    commit_metriques_und_file = file_path / inputFileUndName
    output_file = file_path / outputFile
