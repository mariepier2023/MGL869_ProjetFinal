...
def compute_x(df, lm_intercept, total_point, maxi_point):
    # Ensure df values are numeric and convert to NumPy array
    df_values = df.select_dtypes(include=[np.number]).values

    # Compute the score
    score = np.dot(df_values, lm_intercept)

    # Clip the score to avoid overflow in np.exp
    score = np.clip(score, -500, 500)

    # Compute the probability
    prob = 1 / (1 + np.exp(-score))

    # Ensure label is not empty before accessing
    label = np.where(prob >= 0.5)[0]
    if label.size == 0:
        raise ValueError("No labels found with probability >= 0.5")

    mini_overallpoint = label[0] // (total_point / 2) * (total_point / 2)
    maxi_overallpoint = label[-1] // (total_point / 2) * (total_point / 2)

    return mini_overallpoint, maxi_overallpoint, score, prob
